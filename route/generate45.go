...
func Generate45Handler(c *fiber.Ctx) error {
	...
	var (
		canvasWidth  = 1296.0 * scale
		canvasHeight = 1620.0 * scale
		zoomFactor   = 0.88
		baseFontSize = 35.0
		baseGap      = 60 * scale
	...
	diff := (math.Abs(float64(newHeight)-float64(canvasHeight)) / (float64(canvasHeight) / 2))
	log.Print(diff)

	// check area of image more than 60% of canvas height set to center
	if diff > 0.5 {
